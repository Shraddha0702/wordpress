...
use WPDesk\ShopMagic\Recipe\RecipeException;
use WPDesk\ShopMagic\Recipe\RecipeProvider;

class RecipesTab implements LoggerAwareInterface {
	...
	public function brew_recipe() {
		...
		if ( $this->logger ) {
			$this->logger->info( sprintf( 'Importing recipe %s', $id ) );
		}
		...
		try {
			...
			wp_redirect( wp_get_referer() ?: get_admin_url() );
			exit();
		}

		$post_id = $recipe->import();
		if ( $this->logger ) {
			$this->logger->info( 'Recipe successfully imported.' );
		}
